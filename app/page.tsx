...
export default function Home() {
  ...
  return (
    <main className="geo-app">
      ...
      {selected && <div className="property-drawer" role="dialog" aria-modal="true" aria-label={`Ficha de ${selected.title}`}><button className="drawer-close" onClick={() => setSelected(null)} aria-label="Cerrar detalle de propiedad">×</button>{selected.images.length > 0 && <div className="drawer-gallery"><button className="drawer-main-image" type="button" onClick={() => setGalleryOpen(true)} aria-label="Ampliar galería"><img src={selected.images[galleryIndex]} alt={selected.title} /><span>{galleryIndex + 1} / {selected.images.length}</span></button><div>{selected.images.slice(0, 5).map((image, index) => <button key={image} type="button" aria-label={`Ver imagen ${index + 1}`} className={galleryIndex === index ? "active" : ""} onClick={() => setGalleryIndex(index)}><img src={image} alt="" /></button>)}</div></div>}<div className="drawer-kicker">{selected.operation} · {selected.type}{selected.status ? ` · ${selected.status === "published" ? "Publicado" : selected.status}` : ""}</div><h2>{selected.title}</h2><div className="drawer-price">{selected.currency ?? "USD"} {selected.price.toLocaleString("en-US")}</div><p>{selected.description}</p><div className="drawer-details"><span>⌖ {selected.zone}</span>{selected.area > 0 && <span>▧ {selected.area} m²</span>}{selected.bedrooms != null && <span>⌂ {selected.bedrooms} hab.</span>}{selected.bathrooms != null && <span>♧ {selected.bathrooms} baños</span>}{selected.parkingSpaces != null && <span>▣ {selected.parkingSpaces} estacionamientos</span>}</div><div className="drawer-actions"><button className="contact-btn" type="button" onClick={() => { setSelected(null); setMobileList(false); }}>Ver mapa</button>{selected.slug && <Link className="save-btn" href={`/${selected.slug}`}>Ficha completa</Link>}<button className="save-btn" onClick={() => alert("Propiedad guardada en favoritos")}>♡ Guardar</button></div></div>}
