...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setSelected(null);
      try {
        const { data, error } = await supabase.from("properties").select("id,title,slug,status,listing_type,property_type,price,currency,area_total_m2,address,district,city,description,bedrooms,bathrooms,parking_spaces,lat,lng,property_images(public_url,sort_order,is_cover)").eq("status", "published");
