...
export async function POST(request: Request): Promise<Response> {
  // Browser signal without trusting proxy Host headers; not authorization.
  if (request.headers.get("sec-fetch-site") === "cross-site") return failure("INVALID_ORIGIN", "La solicitud debe iniciarse desde este sitio.", 403);
  ...
  try {
    ...
    if (!url || !key) return failure("PUBLICATION_FAILED", "El servicio de publicación no está disponible.", 503);
    // Dedicated anonymous client: no cookies, user JWT, persistence or refresh.
    ...
    writeAttempted = true;
    const { data: property, error } = await supabase.from("properties").insert({ ...propertyFields, slug, currency: "USD", city: "Piura", region: "Piura", status: "published" }).select("id").single();
    if (error || !property?.id) throw new Error("Property write uncertain");
    for (const [index, file] of files.entries()) {
      const path = `public/${property.id}/${crypto.randomUUID()}.${imageExtensions[file.type as keyof typeof imageExtensions]}`;
